...
local function CreateCheckbox(menuObj, text, keyPath, defaultValue, resetKeys, customOnClick)
	if customOnClick then
		return menuObj:CreateCheckbox(
			text,
			function()
				local value = CheckboxMixin:KeyIsTrue(KrowiBCU_SavedData, keyPath);
				if value == nil then return defaultValue; end
				return value;
			end,
			customOnClick
		...
		return CheckboxMixin:CreateCheckbox(menuObj, text, KrowiBCU_SavedData, keyPath, defaultValue);
	end
end
...
local function CreateParentHeaderCheckbox(parentMenu, text, setKey, headerEntry)
	return CreateCheckbox(parentMenu, text, {"HeaderSettings", setKey}, true, nil, function()
		...
		local currentValue = CheckboxMixin:KeyIsTrue(KrowiBCU_SavedData, keyPath);
		if currentValue == nil then currentValue = true; end
		local newValue = not currentValue;
		CheckboxMixin:SetSelected(KrowiBCU_SavedData, keyPath, newValue);
    ...
    local button = menuObj:CreateRadio(
        ...
        function()
			if type(compareValue) == "number" then
				return CheckboxMixin:KeyEqualsValue(compareValue, KrowiBCU_SavedData, keyPath);
			end
            return CheckboxMixin:KeyEqualsText(text, KrowiBCU_SavedData, keyPath);
        end,
        function()
            CheckboxMixin:OnRadioSelect(compareValue, KrowiBCU_SavedData, keyPath);
        end
    );
...
function menu.CreateMenu(self, menuObj)
	...
	CreateCheckbox(menuObj, addon.L["Show WoW Token"], {"ShowWoWToken"}, true);
